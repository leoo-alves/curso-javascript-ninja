(function (doc, win) {
  ...
  */
  ...
  username ? alert(`Bem vindo ${username}`) : alert(`Bem vindo ${username = 'Desconhecido'}`)
  ...
  */
  ...
  `$inputUsername`.
  */
  var $inputUserName = document.querySelectorAll('input')

  /*
  - Selecione o input de "Email", atribuindo-o à uma variável chamada
  `$inputEmail`.
  */
  var $inputEmail = document.querySelectorAll('input')
  ...
  */
  $inputUserName[0].value = username
  $inputEmail[1].value = email
  ...
  */
  ...
  $button.addEventListener('click', function (event) {
    ...
    if (!isValidEmail($inputEmail[1].value)) {
      alert('Entre com um e-mail válido!')
      botao = false
    }
    if (botao) {
      var confirmacao = confirm('Tem certeza que deseja enviar o formulario')
      if (confirmacao) {
  ...
  */
  ...
  function isValidEmail(email) {
    return /^[\w.+_]+@[\w_]+\.[\w]{2,}(?:\.[\w]{1,2})?$/.test(email)
